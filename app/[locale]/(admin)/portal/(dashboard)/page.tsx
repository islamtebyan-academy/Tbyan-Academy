...
export default async function DashboardHomePage({ params }: DashboardHomeProps) {
  ...
  // Mock visits stats for Chart preview
  const visitsOverview = [
    { label: isRtl ? 'اليوم' : 'Today', count: '142', growth: '+12%' },
    { label: isRtl ? 'هذا الأسبوع' : 'This Week', count: '984', growth: '+8%' },
    { label: isRtl ? 'هذا الشهر' : 'This Month', count: '3,842', growth: '+15%' },
  ];
  ...
  return (
    <div className="space-y-8">
        ...
        {(() => {
          ...
          return [
            ...
            return (
              <div 
                key={i} 
                className="bg-gradient-to-br from-white to-[#FDFAF3]/40 border border-gold-muted/15 p-5 rounded-3xl shadow-[0_8px_30px_rgba(139,115,85,0.05)] relative overflow-hidden group hover:border-gold hover:shadow-[0_15px_35px_rgba(139,115,85,0.1)] transition-all duration-500 flex flex-col justify-between"
              >
                {/* Top Accent Gold Bar */}
                ...
                <div className="flex items-center justify-between gap-4 relative z-10 mb-4">
                  <div className="space-y-1 text-start">
                    <span className="text-[10px] uppercase font-bold tracking-wider text-stone/50 font-ui block">
                      {stat.title}
                    </span>
                    <p className="text-3xl font-bold text-midnight font-primary mt-1 tracking-tight">{stat.value}</p>
                  </div>
                  {/* Luxury double-bordered circular icon badge */}
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center border transition-all duration-500 group-hover:scale-110 relative z-10 p-2.5 shadow-sm ${stat.color}`}>
      ...
      <div className="bg-gradient-to-br from-white to-[#FDFAF3]/40 border border-gold-muted/15 rounded-3xl p-6 text-start shadow-xl relative overflow-hidden pattern-overlay">
        ...
        <div className="overflow-x-auto no-scrollbar">
          {recentStudents && recentStudents.length > 0 ? (
            ...
            <div className="text-center py-12 border border-dashed border-gold/20 rounded-2xl text-stone/50 text-xs font-semibold font-ui bg-[#FDFAF3]/30">
