...
import { 
  Search, X, Calendar, Clock, BookOpen, 
  ArrowLeft, ArrowRight, Share2, 
  Sparkles, Award, GraduationCap, Bookmark, Check
} from 'lucide-react';
import ParallaxBackground from '@/components/ui/ParallaxBackground';

/* ═══════════════════════════════════════════════════════════════
   Articles Database with complete trilingual scholarly summaries
   ═══════════════════════════════════════════════════════════════ */
...
const ARTICLES_DATA: Record<string, Record<string, ArticleTranslation>> = {
  ar: {
      ...
      content: {
        ...
        sections: [
          ...
          },
          {
            ...
            body: 'عند تعارض حديث صحيح من رواية الآحاد مع عمل مستمر لأهل المدينة، يقدم مالك العمل؛ لأن الحديث قد يتطرق إليه السهو أو النسخ أو الخصوصية، بينما العمل العام المتوارث يمثل السنة الظاهرة المأمونة من عوارض النسيان والانقطاع الفردي.'
          }
        ],
    ...
    'ashari-maturidi-theology-methodology': {
      ...
      bookingTopic: 'aqidah',
      content: {
        intro: 'يمثل المذهب الأشعري والماتريدي المنهج الكلامي السائد لجمهور أهل السنة والجماعة عبر القرون. وقد صاغت هاتان المدرستان إطارًا معرفيًا دقيقًا يوفق بين صريح النقل وصحيح العقل، مما مكن الأمة من مواجهة التحديات الفلسفية الوافدة ورد الشبهات الكلامية بمنهجية رصينة.',
        sections: [
          {
        ...
        sections: [
          {
            ...
            body: 'في مقدمة كتابه الأصولي الشهير "المستصفى من علم الأصول"، وضع الإمام الغزالي مقدمة منطقية تزيد على مئة صفحة، مصرحًا بأن هذه المقدمة ليست من صلب علم الأصول ولكنها ضرورية لكل باحث شرعي. وقد أعلن فيها عبارته الشهيرة التي غيرت مسار التعليم الإسلامي واعتمدت المنطق كشرط للضبط الفكري.',
      ...
      content: {
        ...
        sections: [
          ...
          },
          {
            ...
            body: 'Les savants de l\'école malikite divisent la Pratique de Médine en deux catégories principales : la Pratique Transmise (portant sur les poids, les mesures, et l\'appel à la prière), universellement acceptée comme contraignante, et la Pratique Déductive (règles juridiques déduites par les savants de Médine après l\'ère des Compagnons).',
          },
          {
            subtitle: 'Résolution des conflits avec les récits isolés',
            body: 'Lorsqu\'une narration isolée (Hadith Ahad) entre en conflit avec la pratique continue de Médine, l\'Imam Malik préférait la pratique. Il soutenait que les rapports individuels sont susceptibles d\'erreurs ou d\'abrogations, tandis que la pratique communautaire représente la Sunnah manifeste.'
          }
        ],
      ...
      content: {
        ...
        sections: [
          ...
          },
          {
            ...
            body: 'Al-Jurjani a été le pionnier de la connexion entre grammaire (Nahw) et rhétorique (Balaghah). La rhétorique n\'était pas un simple ornement, mais l\'application précise des variations syntaxiques pour transmettre des nuances de sens.',
          },
          {
      ...
      content: {
        ...
        sections: [
          ...
          },
          {
            ...
            body: 'L\'éducation moderne exige d\'inculquer la foi (Aqidah) comme une boussole interne. En enseignant aux enfants l\'esprit critique et en expliquant la sagesse des lois, on développe chez eux une résilience face au contenu numérique.',
          },
          {
...
const BASE_ARTICLES: Omit<Article, 'translation'>[] = [
...
export default function BlogPage() {
  ...
  const activeArticle = useMemo(() => {
    ...
    return articles.find(art => art.slug === activeArticleSlug) || null;
  }, [articles, activeArticleSlug]);
  ...
  return (
    <article className="min-h-screen bg-ivory text-midnight overflow-x-hidden selection:bg-gold-hi/30">
        ...
        <div className="max-w-6xl mx-auto px-6 w-full relative z-10 text-center my-auto flex flex-col justify-center items-center">
          ...
          >
            ...
            <div dir="rtl" className="text-gold-hi font-amiri text-lg md:text-[23px] leading-relaxed mb-1 select-none font-bold">
              {isRtl ? '﴿وَنَزَّلْنَا عَلَيْكَ الْكِتَابَ تِبْيَانًا لِكُلِّ شَيْءٍ﴾' : '﴿وَنَزَّلْنَا عَلَيْكَ الْكِتَابَ تِبْيَانًا لِكُلِّ شَيْءٍ﴾'}
            </div>
      ...
      {/* ── SECTION 3: IMMERSIVE MANUSCRIPT READER DRAWER ── */}
      <AnimatePresence>
        {activeArticle && (
          <>
            {/* Backdrop Mask */}
            <motion.div
              ...
              className="fixed inset-0 bg-midnight/80 backdrop-blur-sm z-40 cursor-pointer"
            />

            {/* Manuscript Panel Drawer */}
            <motion.div
              initial={{ x: isRtl ? '-100%' : '100%' }}
              animate={{ x: 0 }}
              exit={{ x: isRtl ? '-100%' : '100%' }}
              transition={{ type: 'spring', damping: 25, stiffness: 220 }}
              className={`fixed inset-y-0 ${isRtl ? 'left-0 border-r' : 'right-0 border-l'} w-full md:w-[680px] lg:w-[760px] bg-[#FDFAF3] border-gold-hi/25 shadow-[-20px_0_60px_rgba(0,0,0,0.15)] z-50 flex flex-col overflow-hidden text-start`}
            >
              
              {/* Header Panel */}
              <div className="bg-white/80 backdrop-blur-md border-b border-gold-muted/10 p-5 flex items-center justify-between shrink-0 relative">
                <div className="absolute top-0 left-0 right-0 h-[2.5px] bg-gradient-to-r from-transparent via-gold to-transparent" />
                
                <button
                  onClick={() => setActiveArticleSlug(null)}
                  className={`flex items-center gap-1.5 text-stone hover:text-gold transition-colors duration-200 text-xs font-bold ${isRtl ? 'font-cairo' : 'font-dm'}`}
                >
                  {isRtl ? <ArrowRight size={15} /> : <ArrowLeft size={15} />}
                  <span>{labels.btnBack}</span>
                </button>

                <div className="flex items-center gap-3">
                  <button 
                    onClick={() => {
                      navigator.clipboard.writeText(window.location.href + '?article=' + activeArticle.slug);
                    }}
                    className="w-9 h-9 rounded-full bg-stone/5 hover:bg-gold/10 hover:text-gold flex items-center justify-center text-stone transition-all duration-200"
                  ...
                  <button
                    onClick={() => setActiveArticleSlug(null)}
                    className="w-9 h-9 rounded-full bg-stone/5 hover:bg-red-500/10 hover:text-red-500 flex items-center justify-center text-stone transition-all duration-200"
              ...
              <div className="flex-1 overflow-y-auto px-6 py-10 md:px-10 lg:px-12 scroll-smooth">
                <article className="max-w-2xl mx-auto relative">
                  
                  {/* Category & Reading metadata */}
                  <span className={`inline-block text-[9px] uppercase tracking-widest text-gold-hi font-bold mb-3 border-b border-gold-muted/30 pb-1 ${isRtl ? 'font-cairo' : 'font-dm'}`}>
                    {activeArticle.translation.category}
                  </span>
                  
                  {/* Article Title */}
                  <h1 className={`text-2xl md:text-3xl lg:text-[32px] text-midnight font-bold leading-tight mb-6 ${isRtl ? 'font-amiri font-bold' : 'font-cormorant font-semibold'}`}>
                    {activeArticle.translation.title}
                  </h1>

                  {/* Article Date & Authorship metadata */}
                  <div className={`flex items-center gap-5 text-[11px] text-stone/50 mb-10 pb-6 border-b border-gold-muted/15 font-bold ${isRtl ? 'font-cairo' : 'font-dm'}`}>
                    ...
                    <span className="flex items-center gap-1.5">
                      ...
                      {activeArticle.readTime} {labels.readTime}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <GraduationCap size={13} className="text-gold" />
                      {isRtl ? 'هيئة علماء الأزهر' : 'Al-Azhar Faculty Board'}
                    </span>
                  </div>

                  {/* Manuscript Text Styles */}
                  <div className={`text-stone/90 leading-[1.8] text-[15px] space-y-6 ${isRtl ? 'font-noto' : 'font-lora'}`}>
                    
                    {/* Introduction Paragraph */}
                    <p className={`text-base font-medium text-midnight border-l-2 ${isRtl ? 'border-r-2 border-l-0 pr-4' : 'border-l-2 pl-4'} border-gold/40`}>
                      {activeArticle.translation.content.intro}
                    </p>

                    {/* Loop inside sections */}
                    {activeArticle.translation.content.sections.map((section: any, index: number) => (
                      <div key={index} className="pt-4">
                        <h2 className={`text-lg md:text-xl text-midnight font-bold mb-4 pt-4 border-t border-gold-muted/10 ${isRtl ? 'font-amiri font-bold' : 'font-cormorant font-semibold'}`}>
                          {section.subtitle}
                        </h2>
                        <p className="mb-4">
                          {section.body}
                        ...
                        {section.quote && (
                          <div className={`my-6 p-5 rounded-2xl bg-white border ${isRtl ? 'border-r-4 border-r-gold border-l-0' : 'border-l-4 border-l-gold border-r-0'} border-gold-muted/10 text-midnight font-medium italic ${isRtl ? 'font-amiri text-lg' : 'font-lora text-sm'}`}>
                            {section.quote}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="pt-6">
                      <p className="font-semibold text-midnight">
                        {activeArticle.translation.content.conclusion}
                      </p>
                    </div>

                    {/* References & Bibliography list */}
                    {activeArticle.translation.content.references && (
                      <div className="mt-12 pt-8 border-t border-gold-muted/20 text-stone/60 text-xs">
                        <h4 className={`font-bold text-midnight mb-3 ${isRtl ? 'font-cairo' : 'font-dm'}`}>
                          {labels.referencesTitle}
                        </h4>
                        <ul className="list-decimal list-inside space-y-1.5 font-dm">
                          {activeArticle.translation.content.references.map((ref: string, rIdx: number) => (
                            <li key={rIdx}>{ref}</li>
                          ))}
                        </ul>
                      </div>
                  ...
                  {/* Contextual Trial Booking Callout */}
                  <div className="mt-16 bg-gradient-to-br from-[#122038] to-[#091521] border border-gold-hi/20 rounded-3xl p-8 relative overflow-hidden text-center text-parchment shadow-lg">
                    <div className="absolute top-0 left-0 w-24 h-24 bg-gold-hi/5 rounded-full blur-2xl" />
                    <Sparkles className="w-8 h-8 text-gold-hi mx-auto mb-4" />
                    <p className={`text-xs text-parchment/80 leading-relaxed max-w-md mx-auto mb-6 ${isRtl ? 'font-noto' : 'font-lora'}`}>
                      {activeArticle.translation.ctaMessage}
                    </p>
                    <Link
                      href={`/${locale}/book?topic=${activeArticle.translation.bookingTopic}`}
                      className={`btn-gold px-8 py-3.5 rounded-full text-xs font-bold inline-flex items-center gap-2 cursor-pointer shadow-md transition-all duration-300 hover:scale-[1.03] active:scale-[0.97] ${isRtl ? 'font-cairo' : 'font-dm'}`}
                    >
                      <span>{labels.btnBookTrial}</span>
