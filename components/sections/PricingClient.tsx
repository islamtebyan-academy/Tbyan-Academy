...
export default function PricingClient({ settings, locale }: PricingClientProps) {
  ...
  const getPackagePrice = (pkgId: 'starter' | 'optimal' | 'intensive') => {
    ...
    const priceKey = `pricing_${pkgId === 'starter' ? 'plan1' : pkgId === 'optimal' ? 'plan2' : 'plan3'}_price_${classDuration}`;
    const priceSetting = getSetting(priceKey);
    ...
    if (billingCycle === 'monthly') {
      ...
      return {
        ...
        perSession: `$${rate}`,
        savings: `${activeContent.billing.equivalent} $${perMonthEquiv}/${activeContent.billing.perMonth} (${activeContent.billing.save} $${savings.toFixed(0)})`
      };
    }
  };

  const refundTitle = getSetting('pricing_teaser_trial_refund_policy_title');
  const refundDesc = getSetting('pricing_teaser_trial_refund_policy_desc');
