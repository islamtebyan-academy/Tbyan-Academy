...
import { Award, BookOpen, Shield, GraduationCap, Briefcase, Sparkles, Languages, CheckCircle, ArrowLeft } from 'lucide-react';
import { motion, Variants } from 'framer-motion';

// Animations matching the institutional About page rhythm
...
// Complete fallback defaults mapping directly from original hardcoded values
const FALLBACK_DEFAULTS: Record<string, Record<string, any>> = {
  'ahmed-yahya-zakaria': {
    index: 1,
    ar: {
...
export default function TeacherProfileClient({ settings, locale, slug }: TeacherProfileClientProps) {
  const isRtl = locale === 'ar';
  
  // Resolve dynamic fields from DB settings or fallback to standard initial defaults
  const fallbackObj = FALLBACK_DEFAULTS[slug];
  if (!fallbackObj) {
    notFound();
  }

  const idx = fallbackObj.index;
  const fallbackLocale = fallbackObj[locale] || fallbackObj['ar'];

  const getVal = (key: string, defaultVal: string) => {
    return settings[key]?.[locale] || settings[key]?.[locale.toLowerCase()] || defaultVal;
  };

  const name = getVal(`teacher${idx}_name`, fallbackLocale.name);
  const title = getVal(`teacher${idx}_title`, fallbackLocale.title);
  const education = getVal(`teacher${idx}_education`, fallbackLocale.education);
  const specialty = getVal(`teacher${idx}_specialty`, fallbackLocale.specialty);
  const languages = getVal(`teacher${idx}_languages`, fallbackLocale.languages);
  const image = getVal(`teacher${idx}_image`, `/images/teacher_${idx === 1 ? 'ahmed_yahya' : idx === 2 ? 'mohamed_badr' : 'hamada_attia'}.png`);
  const bio = getVal(`teacher${idx}_bio`, fallbackLocale.bio);

  // Helper function to split database lists (newline \n) safely
  const parseList = (key: string, fallbackArr: string[]): string[] => {
    const rawVal = getVal(key, '');
  ...
  const ijazas = parseList(`teacher${idx}_ijazas`, fallbackLocale.ijazas);
  const qualifications = parseList(`teacher${idx}_qualifications`, fallbackLocale.qualifications);
  const experiences = parseList(`teacher${idx}_experiences`, fallbackLocale.experiences);
  const skills = parseList(`teacher${idx}_skills`, fallbackLocale.skills);

  // Custom section for teacher 2 (Mohamed Badr)
  const scholarlyTextTitle = idx === 2 ? getVal('teacher2_scholarlyTextTitle', fallbackLocale.scholarlyTextTitle) : '';
  const scholarlyTexts = idx === 2 ? parseList('teacher2_scholarlyTexts', fallbackLocale.scholarlyTexts) : [];

  // Custom section for teacher 3 (Hamada Attia)
  const specialSectionTitle = idx === 3 ? getVal('teacher3_specialSectionTitle', fallbackLocale.specialSectionTitle) : '';
  const specialSectionText = idx === 3 ? getVal('teacher3_specialSectionText', fallbackLocale.specialSectionText) : '';
  ...
  return (
    <section className="bg-navy-sapphire min-h-screen pt-32 pb-24 relative overflow-x-clip">
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        {/* Content Layout Grid */}
        <motion.div 
          className="grid grid-cols-1 lg:grid-cols-3 gap-10 text-start"
        ...
        >
                ...
                <div className={`space-y-4 text-xs ${isRtl ? 'font-noto' : 'font-lora'}`}>
                  <div className="flex items-start gap-3 p-3 rounded-2xl bg-gold-hi/[0.015] border border-gold-muted/10 hover:border-gold/25 transition-all duration-300">
                    ...
                    <div>
                      ...
                      <span className="text-midnight font-bold leading-normal">{education}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 p-3 rounded-2xl bg-gold-hi/[0.015] border border-gold-muted/10 hover:border-gold/25 transition-all duration-300">
                    ...
                    <div>
                      ...
                      <span className="text-midnight font-bold leading-normal">{specialty}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 p-3 rounded-2xl bg-gold-hi/[0.015] border border-gold-muted/10 hover:border-gold/25 transition-all duration-300">
                    ...
                    <div>
                      ...
                      <span className="text-midnight font-bold leading-normal">{languages}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <motion.div className="bg-ivory border border-gold-muted/20 rounded-[2.5rem] p-8 shadow-[0_20px_50px_rgba(139,115,85,0.08)] relative overflow-hidden" variants={fadeInUp}>
            ...
            {idx === 2 && scholarlyTexts && scholarlyTexts.length > 0 && (
              <motion.div className="bg-ivory border border-gold-muted/20 rounded-[2.5rem] p-8 shadow-[0_20px_50px_rgba(139,115,85,0.08)] relative overflow-hidden" variants={fadeInUp}>
                <div className="flex items-center gap-3 border-b border-gold-muted/10 pb-4 mb-6">
